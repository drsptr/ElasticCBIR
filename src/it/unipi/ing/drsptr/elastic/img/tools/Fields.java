package it.unipi.ing.drsptr.elastic.img.tools;

/*
 * Fields is a class with constants only. These constants represent the fields' name used in the index.
 * @author		Pietro De Rosa
 */
public class Fields {

	public static final int NUMBER_OF_FIELDS = 3;	// total number of fields

	public static final String IMG = "encoded_features";	// the encoded text of the image
	
	public static final String TAGS = "tags";	// the tags assigned to the image
	
	public static final String URI = "uri";		// the uri of the image

}
